...
from XANAF import DEcwHisPErMsG_pb2 , MajoRLoGinrEs_pb2 , PorTs_pb2 , MajoRLoGinrEq_pb2 , sQ_pb2 , Team_msg_pb2, RemoveFriend_Req_pb2, GetFriend_Res_pb2, spam_request_pb2, devxt_count_pb2, dev_generator_pb2, kyro_title_pb2, room_join_pb2
...
def load_credentials_from_file(filename="XANAF.txt"):
    """
    Load UID and password from XANAF.txt file
    """
    try:
        if not os.path.exists(filename):
...
📱 Player will see:
"XANAF LEGACY wants to join your team"
...
async def black666(client_id, key, iv):
    banner_text = "[FF0000][B][C] ERROR , WELCOME TO [FFFFFF]XANAF LG [00FF00]___X³____ BOT ! \n[FFFF00]NEW VERSION NEW FUNCTION !\n[FF0000]TELEGRAM : @XANAF_LEGACY\n\n"     
    fields = {
        1: 5,
        2: {
            1: int(client_id),
...
def get_player_ban_status(uid):
    try:
        url = f"https://xanaf-check-ban.vercel.app/ban?uid={uid}"
...
Region: {server_name}
[FFDD00]°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°
[00FF00][b][c]BOT MADE BY XANAF LEGACY 
"""
            else:
...
def get_player_add(uid):
    try:
        url = f"https://danger-add-friend.vercel.app/adding_friend?uid=4270936858&password=XANAF_GAMER_94IWM_BY_SPIDEERIO_GAMING_JOX82&friend_uid={uid}"
        res = requests.get(url)
        data = res.json()
...
def get_player_basic(uid):
    try:
        url = f"https://xanaf-gamer-super-info-api.vercel.app/get?uid={uid}"
        res = requests.get(url)
        if res.status_code == 200:
            data = res.json()
...
[FF0000]━[00FF00]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[FFD700] ＦＯＬＬＯＷ    ＭＥ   ＩＮ   [87CEEB]TELEGRAM: [FF0000]@XANAF_GAMER
[FF0000]━[00FF00]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]"""
    fields = {
    1: 5,
    2: {
...
"""
            ...
            # For example:
            title_packet = await convert_kyro_to_your_system(uid, chat_id, key, iv, nickname="XANAF GAMER", title_id=title_id)
...
async def noob(target_uid, chat_id, key, iv, nickname="XANAFGAMER", title_id=None):
    """EXACT conversion with customizable title ID"""
    try:
        # Use provided title_id or get random one
        if title_id is None:
...
async def send_all_titles_sequentiallly(uid, chat_id, key, iv, region, chat_type):
    ...
    # Send initial message
    start_msg = f"""[B][C][00FF00] Noobde XANAF GAMER ya meku agar tu noob bolra toh tu g a y hai
...
"""
    ...
    try:
        for index, title_id in enumerate(all_titles):
            ...
            # You'll need to use your existing title sending logic here
            # For example:
            title_packet = await noob(uid, chat_id, key, iv, nickname="XANAFGAMER", title_id=title_id)
                    ...
                    try:
                        ...
                        if "1234567890" in WHITELISTED_UIDS:
                            ...
                            try:
                                ...
                                bot_uid = 13777711848
                                message_text = f" Can't accept Your request Talk to XANAF GAMER"
...
[FFFFFF]📱 CONTACT: [FFFF00]@XANAF_GAMER
[00FF00]»»————-　★　————-««
...
📝 Examples:
/bio Hello World!
/bio 🤖 Bot by XANAF GAMER
/bio Level 70 | Pro Player
/bio Add me: XANAF GAMER
...
"""
                                ...
                                credentials = load_credentials_from_file("XANAF.txt")
                                if not credentials:
...
[b][i][FFC0CB]youtube: XANAF GAMER 99[/b]

[b][c][FFC0CB]subcribe: my_channel[FFFFFF]
 
[b][i][FFA500]telegram: @XANAF_GAMER[/b]

[b][c][FFA500]telegram contact: @XANAF_GAMER[A52A2A]
 
Enjoy the bot my friend.......

[C][B][0000FF] Created by Black666FF
Modified by - XANAFGAMER
"""
...
async def MaiiiinE():
    ...
    print("📁 Loading credentials from XANAF.txt...")
    credentials = load_credentials_from_file("XANAF.txt")
    
    if not credentials:
        print("❌ Failed to load credentials!")
        print("💡 Please create XANAF.txt with your UID and password")
    ...
    os.system('clear')
    print("=" * 50)
    print("🤖 XANAFGAMER BOT - INITIALIZING")
    print("=" * 50)
    ...
    os.system('clear')
    print("🤖 XANAFGAMER BOT - CONNECTING")
    print("=" * 50)
    print("┌────────────────────────────────────┐")
    print("│ ██████████████████████████████████ │")
    print("└────────────────────────────────────┘")
